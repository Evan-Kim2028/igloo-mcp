...
async def test_preview_with_snowflake():
    ...
    try:
        ...
        if result.get('status') == 'success':
            ...
            if rows:
                print(f"   Sample data (first row): {dict(list(rows[0].items())[:3])}{'...' if len(rows[0]) > 3 else ''}")
            else:
                print("   Rows: None")
        else:
    ...
    try:
        ...
        if result.get('status') == 'success':
            ...
            print(f"❌ Simple table preview failed: {result.get('error', 'unknown error')}")
    ...
    try:
        ...
        if result.get('status') == 'success':
            ...
            print(f"❌ Information schema preview failed: {result.get('error', 'unknown error')}")
