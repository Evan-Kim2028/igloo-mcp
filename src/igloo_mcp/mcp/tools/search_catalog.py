...
_SUPPORTED_RESPONSE_MODES: Sequence[str] = (
...
class SearchCatalogTool(MCPTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        normalized_response_mode = response_mode.lower().strip()
        if normalized_response_mode not in _SUPPORTED_RESPONSE_MODES:
            raise MCPValidationError(
                f"Invalid response_mode '{response_mode}'",
                validation_errors=[
                    f"response_mode must be one of: {', '.join(_SUPPORTED_RESPONSE_MODES)}",
                ],
                hints=[
                    "Use response_mode='compact' for token-efficient discovery",
                    "Use response_mode='full' only when you need full DDL and metadata",
                ],
            )
        ...
        if search_all_databases and Path(catalog_dir).name == "data_catalogue":
            ...
            return {
                ...
                "limit": limit,
                "response_mode": normalized_response_mode,
                "results": [_format_result(obj, normalized_response_mode) for obj in all_results],
            }
        ...
        try:
            ...
            return {
                ...
                "limit": limit,
                "response_mode": normalized_response_mode,
                "results": [_format_result(obj, normalized_response_mode) for obj in results],
            }
        ...
        return {
            ...
            "properties": {
                ...
                "response_mode": {
                    "type": "string",
                    "title": "Response Mode",
                    "enum": list(_SUPPORTED_RESPONSE_MODES),
                    "default": RESPONSE_MODE_COMPACT,
                    "description": (
                        "Control response verbosity. "
                        "'compact' (default): discovery essentials only. "
                        "'standard': add comment and column summary. "
                        "'full': include full columns and raw metadata payloads."
                    ),
                    "examples": ["compact", "standard", "full"],
                },
                "object_types": {
                    "type": "array",
                    "title": "Object Types",
...
def _format_result(obj: Any, response_mode: str) -> dict[str, Any]:
    base = {
